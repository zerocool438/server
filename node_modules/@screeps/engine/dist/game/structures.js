...
exports.make = function (_runtimeData, _intents, _register, _globals) {
    ...
    StructureController.prototype.activateSafeMode = register.wrapFn(function () {
        ...
        if (this.safeModeCooldown || this.upgradeBlocked > 0 || this.ticksToDowngrade < C.CONTROLLER_DOWNGRADE[this.level] - C.CONTROLLER_DOWNGRADE_SAFEMODE_THRESHOLD) {
            return C.ERR_TIRED;
        }
    ...
    StructureLab.prototype.boostCreep = register.wrapFn(function (target, bodyPartsCount) {
        ...
        var nonBoostedParts = _(target.body).filter(function (i) {
            return !i.boost && C.BOOSTS[i.type] && C.BOOSTS[i.type][data(_this.id).mineralType];
        }).size();
    ...
    utils.defineGameObjectProperties(StructureSpawn.prototype, data, {
        ...
        spawning: function spawning(o, id) {
            return o.spawning ? new StructureSpawn.Spawning(id) : null;
        }
    });
    ...
    function calcEnergyAvailable(roomObjects, energyStructures) {
        return _.sum(energyStructures, function (id) {
    ...
    StructureSpawn.prototype.spawnCreep = register.wrapFn(function spawnCreep(body, name) {
        ...
        var energyStructures = options.energyStructures && _.uniq(_.map(options.energyStructures, 'id'));
        ...
        if (directions !== undefined) {
            ...
            directions = _.uniq(_.map(directions, function (d) {
                return +d;
            }));
            if (directions.length > 0) {
                // bail if any numbers are out of bounds or non-integers
                if (!_.all(directions, function (direction) {
        ...
        var energyAvailable = energyStructures ? calcEnergyAvailable(runtimeData.roomObjects, energyStructures) : this.room.energyAvailable;
        ...
        intents.set(this.id, 'createCreep', { name: name, body: body, energyStructures: energyStructures, directions: directions });
    ...
    StructureSpawn.Spawning.prototype.setDirections = register.wrapFn(function (directions) {
        ...
        if (_.isArray(directions) && directions.length > 0) {
            ...
            if (!_.any(directions, function (direction) {
                ...
                intents.set(this.spawn.id, 'setSpawnDirections', { directions: directions });
                return C.OK;
            }
        }
        return C.ERR_INVALID_ARGS;
